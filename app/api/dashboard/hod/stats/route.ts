...
export async function GET(request: NextRequest) {
  try {
    const auth = await requireAuth(request, { roles: [Role.HOD] });
    ...
    const [
      ...
      prisma.student.count({
        where: { departmentId: user.departmentId, gender: Gender.FEMALE },
      }),
      prisma.student.count({
        where: { departmentId: user.departmentId, gender: Gender.OTHER },
      }),
    ]);
